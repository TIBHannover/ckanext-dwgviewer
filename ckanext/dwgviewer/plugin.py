...
class DwgviewerPlugin(p.SingletonPlugin):
    ...
    def info(self):
        return {'name': 'dwgviewer',
                'title': p.toolkit._('DWG Viewer'),
                'schema': {'page_url': [ignore_empty, unicode]},
                'iframed': False,
                'icon': 'link',
                'always_available': True,
                'default_title': p.toolkit._('DWG Viewer'),
                }
    ...
    def view_template(self, context, data_dict):
        return 'dwg_view.html'

    def form_template(self, context, data_dict):
        return 'dwg_form.html'